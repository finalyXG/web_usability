function y = expwblcdf(t,A,B,V)
%EXPWBLCDF Modified Weibull Distribution culmulative density function.
%   Y = EXPWBLCDF(t,A,B,V) returns the CDF of the Exponentiated Weibull Distribution
%   (Bathtub Curve) with scale parameter A and shape parameters B, V evaluated at the
%   values in t.  
%
...
%
%   See also EXPWBLPDF, EXPWBLHAZ.
%
%   Author: Andrew O'Connor, AndrewNOConnor(AT)gmail.com

if nargin<1
    error('expwblcdf:TooFewInputs','Insufficient number of parameters.');
end
if nargin < 2
    A = 1;
end
...
try
    ...
    error('stats:expwblcdf:InputSizeMismatch',...
