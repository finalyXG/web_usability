function y = betahaz(t,A,B)
%BETAHAZ Beta hazard rate.
%   Y = BETAHAZ(t,A,B) returns the hazard function of the Beta distribution
...
%
%   See also BETAPDF, BETACDF, BETAFIT, BETAINV, BETALIKE, BETARND, BETASTAT, PDF.
%
%   Author: Andrew O'Connor, occawen(AT)gmail.com

if nargin<1
    error('betahaz:TooFewInputs','Insufficient number of parameters.');
end
if nargin < 2
    A = 1;
end
...
try
y = betapdf(t,A,B)./(1-betacdf(t,A,B));
    
catch
    error('stats:betahaz:InputSizeMismatch',...
