function CDF_autopars

k0=8;
L=[];
hh=[];
mf=findobj('Tag','CDF_MAIN');udata=get(mf,'UserData');X=udata.X;n=length(X);
...
kinf=findobj('Tag','CDF_kerinfo');
set(kinf,'String',kselstr);

KOPT_k=findobj('Tag','CDF_KOPT_k');set(KOPT_k,'Value',k-1);
KOPT_nu=findobj('Tag','CDF_KOPT_nu');set(KOPT_nu,'Value',nu+1);
KOPT_mu=findobj('Tag','CDF_KOPT_mu');set(KOPT_mu,'Value',mu+2);
sf=findobj('Tag','CDF_setting');sdata=get(sf,'UserData');
sdata.K=K;
sdata.h=h;
...
set(sb,'String',hs);
set(sb,'UserData',h);
mainok=findobj('Tag','CDF_setMainOK');
set(mainok,'Enable','on');

set(sf,'UserData',sdata);

if k>2
 CDF_warning('Density estimate can be negative for higher order kernels!');
end
