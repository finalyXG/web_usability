function KDE_autopars

k0=8;
L=[];
hh=[];
mf=findobj('Tag','KDE_MAIN');udata=get(mf,'UserData');X=udata.X;n=length(X);
...
kinf=findobj('Tag','KDE_kerinfo');
set(kinf,'String',kselstr);

KOPT_k=findobj('Tag','KDE_KOPT_k');set(KOPT_k,'Value',k-1);
KOPT_nu=findobj('Tag','KDE_KOPT_nu');set(KOPT_nu,'Value',nu+1);
KOPT_mu=findobj('Tag','KDE_KOPT_mu');set(KOPT_mu,'Value',mu+2);
sf=findobj('Tag','KDE_setting');sdata=get(sf,'UserData');
sdata.K=K;
sdata.h=h;
...
set(sb,'String',hs);
set(sb,'UserData',h);
mainok=findobj('Tag','KDE_setMainOK');
set(mainok,'Enable','on');

set(sf,'UserData',sdata);

if k>2
 KDE_warning('Density estimate can be negative for higher order kernels!');
end
