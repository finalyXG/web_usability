page_elements_masks = page_elements_masks_train;
page_mboxes = page_btn_train_margin_boxes;

page_mboxes_DT = [];
interval = 5;
interval2 = 2;
% imcontour(Dbwmask);
for i = 1 : length(page_mboxes)
    bwmask = im2bw(page_elements_masks{i});
    Dbwmask = bwdist(bwmask,'chessboard');
    
    mboxes = page_mboxes{i};
    mbox_DTs = [];
...
page_mboxes_DT;
page_mboxes_DT_train = page_mboxes_DT;
