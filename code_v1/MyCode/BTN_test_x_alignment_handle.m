...
%     BaggedEnsemble1 = TreeBagger(90,fd_alignment_train(ind1,:),mclasses_labels(ind1),'OOBPred','On','MinLeafSize',1,'CategoricalPredictors',[1:10]);
%     oobErrorBaggedEnsemble1 = oobError(BaggedEnsemble1);
% 
%     BaggedEnsemble0 = TreeBagger(20,fd_alignment_train(ind0,:),mclasses_labels(ind0),'OOBPred','On','MinLeafSize',1,'CategoricalPredictors',[1:10]);
...
mark_x_type2 = 1;
if mark_x_type2 == 1
    BaggedEnsemble_full_x = TreeBagger(90,fd_alignment_train,mclasses_labels,'OOBPred','On','MinLeafSize',1,'CategoricalPredictors',[1:10]);
%     oobErrorBaggedEnsemble_full = oobError(BaggedEnsemble_full);
%     plot(oobErrorBaggedEnsemble_full)
...
acc_ind = 1;
[Y,scores] = predict(BaggedEnsemble_full_x, fd_alignment_test); 
Y = str2num(cell2mat(Y));
ensembel_names = str2num(cell2mat(BaggedEnsemble_full_x.ClassNames));
...
for i = 1 : length(Y)
    ...
    % rule 1 : 
    if fd_alignment_test(i,2) == 0
        cancel_type([1,2,3]) = 1;
    end
    
    % rule 2 :
    if fd_alignment_test(i,14) > 0.45 && fd_alignment_test(i,14) < 0.55 && fd_alignment_test(i,2) ~= 0
        cancel_type([1:9]) = 1; cancel_type(2) = 0;
    end    
 
    % rule 3 :
    if fd_alignment_test(i,1) == 2
        cancel_type([1:9]) = 1; cancel_type(7:9) = 0;
    end 
...
Y1 = str2num(cell2mat(BaggedEnsemble_full_x.ClassNames(mxi)));


% BaggedEnsemble_full.ClassNames
for i = 1 : length(page_split_start_ind)
    page_btn_alignment_test{i} = [];
    pages_alignment_score{i} = [];
    page_btn_alignment_test{i} = Y1(acc_ind : acc_ind + page_split_start_ind(i) - 1 , :);
    pages_alignment_score{i} = scores(acc_ind : acc_ind + page_split_start_ind(i) - 1 , :);
    acc_ind = acc_ind + page_split_start_ind(i);
end
...
% % % % 
% % % % %   /////////////merge step: //////////////
% % % %     [Y,scores] = predict(BaggedEnsemble_full, fd_alignment_test_one);            
% % % %     page_btn_alignment_test{i} = [page_btn_alignment_test{i}; cell2mat(Y)];
...
if Gaussian_Process_Regression_on_off == 1
    ...
    tmp = [];
    tmp{7} = my_GPR(alignment_x_model_data_train{7},alignment_x_model_rs_train{7},nd);
    tmp{9} = my_GPR(alignment_x_model_data_train{9},alignment_x_model_rs_train{9},nd);
    acc_ind = 0;
    for i = 1 : length(page_btn_alignment_test)
        ...
        for j = 1 : size(BTN_test_alignment_tyges,1)
            ...
            page_btn_dx{i} = [page_btn_dx{i} ;tmpv];
...
% % % % % %     for i = 1 : length(page_btn_alignment_test)
% % % % % %         page_btn_dx{i} = [];
% % % % % %         BTN_test_alignment_tyges = str2num(cell2mat(page_btn_alignment_test{i}));
